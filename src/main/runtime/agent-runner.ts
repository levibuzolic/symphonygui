...
export class AgentRunner extends EventEmitter {
  async runIssue(
    ...
    child.once("close", (code) => {
      ...
      emitTransportUpdate(
        "transport_closed",
        `child process closed${code === null ? "" : ` (${code})`}`,
      );
    });
    child.once("exit", () => {
      transportClosed = true;
    ...
    const stream = async (line: string) => {
      ...
      if (message.id === 2) {
        threadId = String(
          (message.result as { thread?: { id?: string } } | undefined)?.thread?.id ?? "",
        );
        ...
        if (!wroteTurnStart) {
          emitTransportUpdate(
            "transport_closed",
            `turn/start skipped because stdin closed${transportError ? ` (${transportError})` : ""}`,
          );
        }
      }
      ...
      if (event) {
        ...
        if (event.event === "client/tool_call" && adapter.executeDynamicTool) {
          ...
          if (!wroteToolResult) {
            emitTransportUpdate(
              "transport_closed",
              `tool result skipped because stdin closed${transportError ? ` (${transportError})` : ""}`,
            );
          }
        }
      }
