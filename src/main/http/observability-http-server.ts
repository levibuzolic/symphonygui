...
export class ObservabilityHttpServer {
  ...
  constructor(private store: ObservabilityStore, private orchestrator: Orchestrator) {}
  ...
  async start(preferredPort: number) {
    ...
    app.get('/api/v1/:identifier', async (request, response) => response.json(await this.orchestrator.getIssueDetails(request.params.identifier)))
