...
export class LocalKanbanStore {
  ...
  initializeDefaults() {
    const boardCount = this.db.prepare("SELECT COUNT(*) as count FROM boards").get() as {
      count: number;
    };
  ...
  getBoard(boardId?: string | null): KanbanBoardPayload | null {
    const boardRow = boardId
      ...
      : (this.db.prepare("SELECT * FROM boards ORDER BY updated_at DESC LIMIT 1").get() as
          | BoardRow
          | undefined);
    ...
    const columns = (
      this.db
        ...
        .all(boardRow.id) as unknown as ColumnRow[]
    ).map(mapColumn);
    const tasks = (
      this.db
        .prepare(
          "SELECT * FROM tasks WHERE board_id = ? AND archived_at IS NULL ORDER BY position ASC, updated_at DESC",
        )
        .all(boardRow.id) as unknown as TaskRow[]
  ...
  updateBoard(input: UpdateKanbanBoardInput) {
    ...
    this.db
      ...
      .run(input.name.trim() || "My Tasks", now, input.boardId);
    return this.getBoard(input.boardId)!;
  }

  createColumn(input: CreateKanbanColumnInput) {
    const positionRow = this.db
      .prepare("SELECT COALESCE(MAX(position), -1) as position FROM columns WHERE board_id = ?")
      .get(input.boardId) as { position: number };
  ...
  updateColumn(input: UpdateKanbanColumnInput) {
    const existing = this.db.prepare("SELECT board_id FROM columns WHERE id = ?").get(input.id) as
      | { board_id: string }
      | undefined;
  ...
  updateTask(input: UpdateKanbanTaskInput) {
    const existing = this.db.prepare("SELECT board_id FROM tasks WHERE id = ?").get(input.id) as
      | { board_id: string }
      | undefined;
  ...
  moveTask(input: MoveKanbanTaskInput) {
    const existing = this.db
      .prepare("SELECT board_id, column_id FROM tasks WHERE id = ?")
      .get(input.taskId) as { board_id: string; column_id: string } | undefined;
    ...
    nextOrder.splice(Math.min(Math.max(input.targetPosition, 0), nextOrder.length), 0, {
      id: input.taskId,
    });
    ...
    try {
      this.db.exec("BEGIN");
      this.db
        .prepare("UPDATE tasks SET column_id = ?, updated_at = ? WHERE id = ?")
        .run(input.targetColumnId, now, input.taskId);
      nextOrder.forEach((task, index) => {
        this.db
          .prepare("UPDATE tasks SET position = ?, updated_at = ? WHERE id = ?")
          .run(index, now, task.id);
      });
  ...
  archiveTask(taskId: string) {
    const existing = this.db.prepare("SELECT board_id FROM tasks WHERE id = ?").get(taskId) as
      | { board_id: string }
      | undefined;
  ...
  private mapTask(task: TaskRow): KanbanTask {
    const labels = (
      this.db
        ...
        .all(task.id) as unknown as Array<{ label: string }>
    ).map((row) => row.label);
    return {
      id: task.id,
  ...
  private nextIdentifier() {
    const row = this.db
      .prepare(
        "SELECT identifier FROM tasks WHERE identifier LIKE 'LOCAL-%' ORDER BY CAST(SUBSTR(identifier, 7) AS INTEGER) DESC LIMIT 1",
      )
  ...
  private nextTaskPosition(columnId: string) {
    const row = this.db
      .prepare(
        "SELECT COALESCE(MAX(position), -1) as position FROM tasks WHERE column_id = ? AND archived_at IS NULL",
      )
...
function toBranchName(identifier: string, title: string) {
  return `${identifier.toLowerCase()}-${title
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
