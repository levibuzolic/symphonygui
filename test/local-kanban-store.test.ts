...
describe('local kanban store', () => {
  it('seeds a default board with a sample task and supports task creation', () => {
    ...
    expect(board?.columns.map((column) => column.name)).toEqual(['Inbox', 'Todo', 'In Progress', 'Blocked', 'Done'])
    ...
    expect(next.tasks.find((task) => task.identifier === 'LOCAL-2')?.labels).toEqual(['runtime', 'sqlite'])
