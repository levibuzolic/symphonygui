...
describe('orchestrator shutdown', () => {
  it('does not append tick logs after stop when an in-flight poll resolves late', async () => {
    ...
    const orchestrator = new Orchestrator(workflowLoader as never, registry as never, store, new RuntimeLogger())

    ;(orchestrator as unknown as { config: ReturnType<typeof createConfig> }).config = createConfig()
    ;(orchestrator as unknown as { workflowDefinition: { config: Record<string, never>; promptTemplate: string; sourcePath: string; loadedAt: string } }).workflowDefinition = {
      config: {},
      promptTemplate: 'noop',
    ...
    expect(appendLog).not.toHaveBeenCalledWith(expect.objectContaining({ message: 'Fetched candidate issues' }))
  })
})
